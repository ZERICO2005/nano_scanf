...
#define CONSUME_WHITESPACE() while (isspace((unsigned char)*buf)) { buf++; }
        ...
        switch (*fmt) {
            ...
            /* string */ {
                ...
                for (; max_width --> 0;) {
                    ...
                    if (string_format && isspace((unsigned char)*buf)) {
                        break;
                    }
                    buf++;
                }
...
int nano_sscanf(const char * NANO_SCANF_restrict buffer, const char * NANO_SCANF_restrict format, ...)
{
    va_list vlist;
    va_start(vlist, format);
    int ret = nano_vsscanf(buffer, format, vlist);
    va_end(vlist);
    return ret;
}
